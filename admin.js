...
/*=====================================
        SUSPEND USER
=====================================*/
async function toggleUserSuspension(index) {
    const userId = users[index].userId || users[index].id;
    const newStatus = users[index].status === "Suspended" ? "Active" : "Suspended";
    const confirmMsg = newStatus === "Suspended"
        ? "Suspend this user? Their campaigns will be hidden from public view."
        : "Reactivate this user? Their campaigns stay hidden until manually restored.";
    if (!confirm(confirmMsg)) return;

    users[index].status = newStatus;
    loadUsers();

    const response = await fetchAPI("updateUserStatus", { userId: userId, status: newStatus, token: localStorage.getItem("adminToken") });
    if (!response || response.status !== "Success") alert("❌ Failed: " + (response?.message || "Unknown error"));
}
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="table-actions">
                   ...
                   <button onclick="window.location.href='campaigns.html?id=${campaign.campaignId}'">📄 Details</button>
...
async function blockCampaignPrompt(campaignId) {
    const reason = prompt("Enter a reason for blocking this campaign:");
    if (reason === null) return;
    if (reason.trim() === "") { alert("A reason is required to block a campaign."); return; }
    const response = await fetchAPI("blockCampaign", { campaignId: campaignId, reason: reason.trim(), token: localStorage.getItem("adminToken") });
    if (response && response.status === "Success") { alert("🚫 Campaign blocked."); loadCampaigns(); }
    else { alert("❌ " + (response?.message || "Failed to block.")); }
}

async function unblockCampaign(campaignId) {
    if (!confirm("Unblock this campaign and make it public again?")) return;
    const response = await fetchAPI("updateCampaignStatus", { campaignId: campaignId, status: "Active", token: localStorage.getItem("adminToken") });
    if (response && response.status === "Success") { alert("🔓 Campaign unblocked."); loadCampaigns(); }
    else { alert("❌ " + (response?.message || "Failed to unblock.")); }
}
...
${user.status === "Suspended"
  ? `<button onclick="toggleUserSuspension(${index})">✅ Activate</button>`
  : `<button onclick="toggleUserSuspension(${index})">🚫 Suspend</button>`
}
...
document.getElementById("logoutBtn")?.addEventListener("click", async () => {
    try {
        await fetch(API_URL, { method: "POST", body: JSON.stringify({ action: "logoutSession", data: { token: localStorage.getItem("adminToken") } }) });
...
async function emailUser(index) {
    const email = users[index].email;
    const subject = prompt("Email subject:");
    if (!subject) return;
    const message = prompt("Message:");
    if (!message) return;
    const response = await fetchAPI("sendAdminEmail", { to: email, subject: subject, message: message, token: localStorage.getItem("adminToken") });
    alert(response?.status === "Success" ? "✅ Email sent to " + email : "❌ " + (response?.message || "Failed."));
}