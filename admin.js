...
<td>${campaigns.filter(c=>c.ownerId===user.userId).length}</td>

<td>${donations.filter(d=>d.userId===user.userId).length}</td>
...
<button onclick="viewUser(${index})">

👁 View

</button>

<button onclick="emailUser(${index})">
...
document.getElementById("popupUserId").innerText=user.userId;

document.getElementById("popupName").innerText=user.fullName;

document.getElementById("popupEmail").innerText=user.email;

document.getElementById("popupPhone").innerText=user.mobile||"-";

document.getElementById("popupDob").innerText=user.dob||"-";

document.getElementById("popupJoin").innerText=user.joinDate||"-";

document.getElementById("popupCampaigns").innerText=
campaigns.filter(c=>c.ownerId===user.userId).length;

document.getElementById("popupDonations").innerText=
donations.filter(d=>d.userId===user.userId).length;
...
/*=====================================
        HIDE CAMPAIGN LOGIC (FIXED)
=====================================*/
function hideCampaign(index) {
    if(!campaigns[index]) return;
    
    // Toggle between Hidden and Active
    if(campaigns[index].status === "Hidden") {
        campaigns[index].status = "Active";
        alert("👁 Campaign Unhidden! It is now Active.");
    } else {
        campaigns[index].status = "Hidden";
        alert("🙈 Campaign Hidden! It will not show to the public.");
    }
...
function viewCampaign(index){
    ...
    document.getElementById("campaignPopupDate").innerText = campaign.createdDate || campaign.date || "-";
...
function loadDonations() {
    ...
    paginated.forEach(obj => {
        ...
        // Headers: Donation ID | Campaign ID | Donor | Amount | Status | Date | Actions
        tr.innerHTML = `
            <td>${donation.id || "-"}</td>
            <td>${donation.campaignId || donation.campaign || "-"}</td>
            <td>${donation.name || donation.userId || "Anonymous"}</td>
...
const transactionId =
"GBTX"+Date.now().toString().slice(-8);
...
(c.campaignId||"")===(donation.campaignId||donation.campaign)
);
...
Array.from(paymentHistoryTable.rows).forEach(row=>{
