...
/*=====================================
        AUTO LOGIN CHECK
=====================================*/
// Prevent auto-login if the user just signed up
if (localStorage.getItem("justSignedUp") === "true") {
    // Clear the flag so they can auto-login normally on future visits
    localStorage.removeItem("justSignedUp"); 
    // Ensure they aren't marked as logged in yet so they must type credentials
...
if(verifyBtn) {
    verifyBtn.onclick = () => {
        ...
        if(email === "" || pass === "") { alert("Please enter both ID/Email and Password."); return; }
...
if(adminLoginForm) {
    adminLoginForm.addEventListener("submit", async (e) => {
        ...
        submitBtn.innerHTML = "Verifying... ⏳"; submitBtn.disabled = true;
        try {
            const response = await fetch(API_URL, {
                method: "POST",
            ...
            if (result.status === "Success") {
                ...
                alert("❌ " + result.message);
                submitBtn.innerHTML = "🛡️ Login as Admin"; submitBtn.disabled = false;
            }
        } catch (error) {
